...
const carImages: { id: number; src: string }[] = [
...
const ImageGrid = () => {
  const [items, setItems] = useState(carImages);
  const [currentImage, setCurrentImage] = useState<number>(0);

  const handleImages = (index: { id: number; src: string }) => {
    const newItems = [index, ...items.filter((i) => i.id !== index.id)];
    setItems(newItems);
    setCurrentImage(index.id);
  };
